 ...
 */
    ...
    var svgElements = makeMap(
        "animate,animateColor,animateMotion,animateTransform,circle,defs," +
                ...
                if (
                    ...
                    (key === "font-size" &&
                        (value === "xx-small" ||
                            ...
                            value === "smaller" ||
                            value.match(
                                /[0-9]*\.?[0-9]*(px|em|rem|mm|q|cm|in|pt|pc|%)/
                            ))) ||
                    ((key === "width" || key === "height") &&
                        value.match(/[0-9\.]*(px|em|rem|%)/)) || // Reference #520
                    (key === "direction" &&
                        value.match(/^ltr|rtl|initial|inherit$/))
                )
                    result += key + ": " + value + ";";
            }
        });
    ...
    function validCustomTag(tag, attrs, lkey, value) {
        ...
        if (tag === "img" && attrs["ta-insert-video"]) {
            if (
                lkey === "ta-insert-video" ||
                lkey === "allowfullscreen" ||
                lkey === "frameborder" ||
                (lkey === "contenteditable" && value === "false")
            )
                return true;
        }
        return false;
            ...
            start: function (tag, attrs, unary) {
                ...
                if (!ignore && validElements[tag] === true) {
                    ...
                    angular.forEach(attrs, function (value, key) {
                        ...
                        if (
                            (lkey === "style" &&
                                (value = validStyles(value)) !== "") ||
                            validCustomTag(tag, attrs, lkey, value) ||
